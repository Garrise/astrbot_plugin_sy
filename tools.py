...
class ReminderTools:
    ...
    async def set_reminder(self, event: Union[AstrMessageEvent, Context], text: str, datetime_str: str, user_name: str = "用户", repeat: str | None = None, holiday_type: str | None = None, group_id: str | None = None, at_all: bool = False):
        '''设置一个提醒
        ...
        try:
            ...
            async for result in self.processor.process_add_item(
                event, 'reminder', text, parsed_datetime_str, None, repeat, holiday_type, group_id, time_already_parsed=True, user_name=user_name, at_all=at_all
            ):
    ...
    async def set_task(self, event: Union[AstrMessageEvent, Context], text: str, datetime_str: str, repeat: str | None = None, holiday_type: str | None = None, group_id: str | None = None, user_name: str | None = None, at_all: bool = False):
        ...
        try:
            ...
            result_message = None
            async for result in self.processor.process_add_item(
                event, 'task', text, parsed_datetime_str, None, repeat, holiday_type, group_id, time_already_parsed=True, user_name=user_name, at_all=at_all
            ):
                # event.plain_result() 返回的是MessageEventResult对象
                # 使用get_plain_text()方法提取纯文本消息
                if hasattr(result, 'get_plain_text'):
